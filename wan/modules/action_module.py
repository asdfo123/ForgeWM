...
class ActionModule(nn.Module):
    ...
    def forward(self, x, tt, th, tw, mouse_condition=None, keyboard_condition=None, block_mask_mouse=None, block_mask_keyboard=None, is_causal=False, kv_cache_mouse=None, kv_cache_keyboard=None, start_frame=0, use_rope_keyboard=True, num_frame_per_block=3):
        ...
        if not ((N_feats == tt and ((is_causal and kv_cache_mouse == None) or not is_causal)) or ((N_frames - 1) // self.vae_time_compression_ratio + 1 == start_frame + num_frame_per_block and is_causal)):
            import os
            if os.environ.get("RANK", "0") == "0":
                print(f"[DEBUG action_module assert] N_frames={N_frames} N_feats={N_feats} tt={tt} start_frame={start_frame} num_frame_per_block={num_frame_per_block} is_causal={is_causal} kv_cache_mouse_is_None={kv_cache_mouse is None}", flush=True)
        ...
        if self.enable_mouse and mouse_condition is not None:
            ...
            group_frame_count = group_mouse.shape[1]

            S = th * tw 
            group_mouse = group_mouse.unsqueeze(-1).expand(B, group_frame_count, pad_t, C, S)
            group_mouse = group_mouse.permute(0, 4, 1, 2, 3).reshape(B * S, group_frame_count, pad_t * C) 
            ...
            if is_causal:
                if kv_cache_mouse is None:
                    ...
                    sink_size = 0
                    max_attention_size = self.local_attn_size if self.local_attn_size != -1 else 15
                    sink_tokens = sink_size * 1
                    kv_cache_size = kv_cache_mouse["k"].shape[1]
                    num_new_tokens = q.shape[1]
                    

                    # Guard eviction on sliding-window mode only (mirror of main
                    # kv_cache fix in causal_model.py).  In full-causal (-1), the
                    # cache is sized to fit the whole rollout; rolling it in-place
                    # during training breaks backward.
                    if self.local_attn_size != -1 and (current_end > kv_cache_mouse["global_end_index"].item()) and (
                    ...
                    # Clone slices under grad to isolate saved_tensors from
                    # future in-place writes to kv_cache_mouse (see main
                    # causal_model.py for rationale).
                    _km = kv_cache_mouse["k"][:, max(0, local_end_index - max_attention_size):local_end_index]
                    _vm = kv_cache_mouse["v"][:, max(0, local_end_index - max_attention_size):local_end_index]
            ...
            if use_rope_keyboard: 
                ...
                if is_causal:
                    if kv_cache_keyboard is None:
                        ...
                        if self.local_attn_size != -1 and (current_end > kv_cache_keyboard["global_end_index"].item()) and (
                ...
                if is_causal:
                    if kv_cache_keyboard is None:
                        ...
                        assert k.shape[1] == num_frame_per_block
                        sink_size = 0
                        local_attn_size = self.local_attn_size if self.local_attn_size != -1 else 15
                        max_attention_size = self.local_attn_size if self.local_attn_size != -1 else 15
                        sink_tokens = sink_size * 1
                        kv_cache_size = kv_cache_keyboard["k"].shape[1]
                        num_new_tokens = k.shape[1]
                        ...
                        _kk = kv_cache_keyboard["k"][:, max(0, local_end_index - max_attention_size):local_end_index]
                        _vk = kv_cache_keyboard["v"][:, max(0, local_end_index - max_attention_size):local_end_index]
                        if torch.is_grad_enabled():
                            _kk = _kk.clone()
                            _vk = _vk.clone()
                        attn = flash_attn_func(
